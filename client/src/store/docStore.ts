...
interface IDocStore {
  hydrated: boolean;
    ...
    getDocData(id:string,key:string):Promise<{
        title:string,
        content:string,
        id:string,
        key:string
    }>
    getviewDocData(id:string,key:string):Promise<{
        title:string,
        content:string,
        id :string
    }>
...
export const useDocStore = create<IDocStore>()(
  persist(
    immer((set) => ({
      ...
      async listDocs(){
        let isLoaded = true
        const result =  await (await apiAxios.get("/docs/list").catch(()=>{isLoaded = false}))
      ...
      async getDocData(id:string,key:string){
        let isLoaded = true
        const result =  await (await apiAxios.get(`/docs/${id}/${key}`).catch((e)=>{
            console.log(e)
      ...
      async updateTitle(id:string,key:string,title:string){
        let isLoaded = true
         const result = await (await apiAxios.put(`/docs/${id}/${key}`,{title}).catch((e)=>{
            console.log(e)
      ...
      async deleteDoc(id:string,key:string){
        ...
        const result =  await (await apiAxios.delete(`/docs/${id}/${key}`).catch((e)=>{
            console.log(e)
